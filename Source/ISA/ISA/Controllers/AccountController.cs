            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    _emailSender.Send(new EmailMessage
                    {
                        ...
                        ToAddresses = new List<EmailAddress> { new EmailAddress { Address = model.Email } },
                        FromAddresses = new List<EmailAddress> { new EmailAddress { Address = AdminMail } }
                    });
            ...
            if (model.SelectedProvider == "Email")
            {
                _emailSender.Send(new EmailMessage
                {
                    ...
                    ToAddresses = new List<EmailAddress>
                        {
                            new EmailAddress { Address = await _userManager.GetEmailAsync(user) }
                        },
                    FromAddresses = new List<EmailAddress> { new EmailAddress { Address = AdminMail } }
                });
            }
